Namespace MyControls.Rdms.Tables.Unit
...
#Region "User Functions"
        ...
        Private Sub SaveScript()
            ...
            Dim IsUnitCodeExisted As Boolean = NasLib.Database.MySql.Provider.Table.OfficeUnit.IsExistedInitial(txtCode.Text, ProvidersConnectionString)
            If IsUnitCodeExisted Then
                txtCode.BackColor = Drawing.Color.Red
                txtCode.ToolTip = String.Format("Unit code ""{0}"" is already existed.", txtCode.Text)

                Exit Sub
            End If

            '' 2. ### Save all Textboxes values in database ###
            NasLib.Database.MySql.Provider.Table.OfficeUnit.UnitAdd(txtName.Text, txtCode.Text, BranchId, DepartmentId, DivisionId, ProvidersConnectionString)
...
#Region "Functions"
        ...
        Private Sub LoadTableData()
            Dim MyFieldname As String() = {"id", "unit", "initial"}

            Dim UnitTable As DataTable = NasLib.Database.MySql.Provider.Table.OfficeUnit.GetUnitsByDivisionId(DivisionId, MyFieldname, ProvidersConnectionString)

            Dim UnitCount As Integer = UnitTable.Rows.Count

            If UnitCount > 0 Then
                For i As Integer = 0 To UnitCount - 1
                    Dim MyControls As MyControls.Rdms.Tables.Unit.RowItem = LoadControl("~\MyControls\Rdms\Tables\Unit\RowItem.ascx")
                    MyControls.No = i + 1
                    MyControls.UnitId = UnitTable(i)("id")
                    MyControls.UnitName = UnitTable(i)("unit")
                    MyControls.UnitCode = UnitTable(i)("initial")
